...
design_mat<-function(ts_data,data_edges,alphaOrder,betaOrder,net,lead_lag_mat,globalalpha=TRUE,lead_lag_weights=FALSE){
  ...
  if (sum(betaOrder) > 0) {
    # OLD: wei_mat <- lapply(1:max(betaOrder),function(x)Matrix(0,nedges,nedges,doDiag = FALSE))
    # NEW: precompute weight matrices upfront for equal-weights case; initialise empty sparse matrices for lead-lag case
    if (!lead_lag_weights) {
      wei_mat <- nei_wei_mat(net, data_edges, max(betaOrder), nedges)
    } else {
    ...
    for (edg in 1:nedges){
      ...
      # OLD per-edge neighbour computation (runs for every edge; now only needed for lead_lag_weights case)
      # # GET NEIGHBOUR EDGES
      ...
      # NEW: per-edge neighbour computation only needed when using lead-lag weights
      if (lead_lag_weights) {
        nodess <- c(data_edges[edg,1],data_edges[edg,2])
        nei <- edge_neighbors(net,max(betaOrder),nodess)
        wei <- list()
        if ((!is.null(nei)) & (length(nei) > 0)) {
          for (ilagg in 1:alphaOrder){
            if (betaOrder[ilagg]>=1){
      ...
      # OLD filling block: uses per-edge nei/wei, handles both equal-weights and lead-lag in one code path
      ...
      #                   }
      #                   sub.val <- vts.cut[which(!is.na(vts.cut[,t])),t] %*% new.wei
      #                   vts.cut[which(is.na(vts.cut[,t])),t] <- sub.val
      #                 }
      #               }
      #             }
      #             dmat[((predt * (edg - 1) + 1):(predt * edg)), betaLoc] <- as.vector(t(vts.cut) %*% wei[[stag]])
      #             wei_mat[[stag]][edg_loc,edg] <- wei[[stag]]
      #           } else { # CASE WHERE ONLY ONE stag-STAGE NEIGHBOUR EDGE
      #             edg_loc <- edge_matching(data_edges, nei[[stag]])
      ...
      #             wei_mat[[stag]][edg_loc,edg] <- wei[[stag]]
      #           }
      #         } else { # case where no neighbours of stag size
      #           dmat[((predt * (edg - 1) + 1):(predt * edg)), betaLoc] <- 0
      #         }
      #       } # end for stage
      ...
      # NEW filling block: two separate paths for equal-weights vs lead-lag
      if (!lead_lag_weights) {
        # EQUAL-WEIGHTS CASE: read neighbour weights directly from precomputed wei_mat columns
        for (ilag in 1:alphaOrder){
          if (betaOrder[ilag]>=1){
            for (stag in 1:betaOrder[ilag]){
              betaLoc <- paste("beta", ilag, ".stage", stag, sep = "")
              w_col <- as.vector(wei_mat[[stag]][,edg])
              neigh_idx <- which(w_col != 0)
              if (length(neigh_idx) == 0) {
                dmat[((predt * (edg - 1) + 1):(predt * edg)), betaLoc] <- 0
              } else {
                w_neigh <- w_col[neigh_idx]
                vts.cut <- ts_data[neigh_idx, ((alphaOrder + 1 - ilag):(predt + (alphaOrder - ilag))), drop=FALSE]
                for (t in 1:ncol(vts.cut)) {
                  if (any(is.na(vts.cut[,t]))) {
                    if (all(is.na(vts.cut[,t]))) {
                      vts.cut[,t] <- 0
                    } else {
                      non_na <- which(!is.na(vts.cut[,t]))
                      new.wei <- w_neigh[non_na] / sum(w_neigh[non_na])
                      vts.cut[which(is.na(vts.cut[,t])),t] <- as.vector(vts.cut[non_na, t, drop=FALSE] %*% new.wei)
                    }
                  }
                }
                dmat[((predt * (edg - 1) + 1):(predt * edg)), betaLoc] <- as.vector(t(vts.cut) %*% w_neigh)
              }
            }
          }
        }
      } else {
        # LEAD-LAG WEIGHTS CASE: use per-edge nei/wei computed above; also populate wei_mat
        if ((!is.null(nei)) & (length(nei) > 0)) {
          for (ilag in 1:alphaOrder){
            if (betaOrder[ilag]>=1){
              for (stag in 1:betaOrder[ilag]){
